...
function countTask(obj,deviceArr){
	...
	if(obj.time&&obj.time[0]){
		...
		on AUTH.taskId=TASK.id where  TASK.isSend=1 and TASK.createtime between '${starttime}' and '${endtime}' ${str4}`
	}else{
		sql=`select count(*) as count from ROMS_SZ_TASK TASK join ROMS_SZ_TASKAUTH AUTH 
		on AUTH.taskId=TASK.id where  TASK.isSend=1 and TASK.pass !=3 ${str4}`
	}
	
	log.info("countTask:sql="+sql);
	return new Promise((resolve,reject)=>{
		pool.query(sql,(err,rows,field)=>{
			if(err)reject(err)
			if(rows){
...
async function showceshitask(req,res){
	let taskgroup=await gettaskId(req)
...
function getcounttaskId(obj){
	return new Promise((resolve,reject)=>{
		...
		//判断：3 所有，未检 0、已检未上传 1、已检上传2；初始未空，表示选择所有任务
...
function gettaskId(req){
	...
	return new Promise((resolve,reject)=>{
		...
		let start = req.body.nowPage*10
		let str = ``
		if(req.body.searchForm.city.length){
			str+=` AND task.district='${req.body.searchForm.city[2]}'`
		}
		if(req.body.searchForm.user){
			str+=` AND  name.staffName='${req.body.searchForm.user}'`
		}
		if(req.body.searchForm.keyword){
			str+=`AND task.checkUnitName='${req.body.searchForm.keyword}'`
		}
		
		//判断：3 所有，未检 0、已检未上传 1、已检上传2；初始未空，表示选择所有任务
		if(req.body.searchForm.taskState==0){
			str+=`AND isDone=0`;
		}else if(req.body.searchForm.taskState==1){
			str+=`AND isDone=1 AND data is null`;
		}else if(req.body.searchForm.taskState==2){
			str+=`AND isDone=1 AND data is not null`;
		}else {
				//全部
			str+=``;
		}
		...
		//判断是未审核还是其他状态
		if(req.body.searchForm.state == 0){
			str+=` AND (task.pass=0 or task.pass=3)`;
		}else{
			str+=` AND task.pass=${req.body.searchForm.state}`;
		}
		
		let sql = ``;  
		
		sql = `select project.docDisplayUrl,name.staffName,task.checkDeviceName,task.checkUnitAddress,task.checkUnitName,task.checkUnitContact,task.checkUnitPhone,task.docNum,
		task.expectTime,task.isDone,length(task.data) as datalength,task.projectNum,task.note1,task.district from ROMS_SZ_TASKSTAFF staff
		join ROMS_SZ_TASK task on task.id=staff.taskId join ROMS_SZ_PROJECT project on project.id=task.projectId join ROMS_SZ_STAFF name on name.id=project.staffId
		where staff.type=0  AND staff.staffId='${id}' ${str} limit ${start},10`
		log.info( 'gettaskId sql =' +sql)
		...
		pool.query(sql,(err,rows,field)=>{
			...
			{
				if(rows[i].isDone==0)
				rows[i].taskState=0 //未检
				else if(rows[i].isDone==1 && rows[i].datalength==null)
				rows[i].taskState=1 //已检未上传
				else
				rows[i].taskState=2 //已检上传
			}
			resolve(rows)
		})
	})
...
async function makeTaskDoc(req,res) {
	log.info("makeTaskDoc:");
	log.info("makeTaskDoc:"+req.body.taskId);
	//let datax = ""
	request.post({
	url:'http://127.0.0.1:8080/roms/Json/ROMS_SZ_TASKAction!packTaskZip',
	headers:{
...
function queryTasksByCtime(obj,deviceArr){
	...
	if(obj.time&&obj.time[0]){
		starttime = parseLittleTime(obj.time[0])
		endtime = parseMoreTime(obj.time[1])
		sql=`select TASK.reason,TASK.docReason,TASK.projectNum,TASK.docNum,TASK.id,TASK.createtime,TASK.checkUnitName,TASK.pass,TASK.measureDataPdfUrl as pdfUrl from ROMS_SZ_TASK TASK join ROMS_SZ_TASKAUTH AUTH 
		on AUTH.taskId=TASK.id
		where TASK.createtime between '${starttime}' and '${endtime}' ${str4} and TASK.isSend=1 order by
		TASK.createtime desc limit ${indexStart},${indexEnd}`
	}else{
		sql=`select TASK.reason,TASK.docReason,TASK.projectNum,TASK.docNum,TASK.id,TASK.createtime,TASK.checkUnitName,TASK.pass,TASK.measureDataPdfUrl as pdfUrl from ROMS_SZ_TASK TASK join ROMS_SZ_TASKAUTH AUTH 
		on AUTH.taskId=TASK.id
		where 1=1 ${str4} and TASK.pass !=3 and TASK.isSend=1 order by TASK.createtime desc limit ${indexStart},${indexEnd}`
	}
	log.info("queryTasksByCtime sql="+sql);
	return new Promise((resolve,reject)=>{
		console.log(sql)
		pool.query(sql,(err,rows,field)=>{
			if(err)reject(err)
...
function taskHasSend(obj){
	...
	log.info("taskHasSend sql="+sql)
	let promises = obj.taskId.map((item)=>{
		return new Promise((resolve,reject)=>{
			pool.query(sql,[item],(err,rows,field)=>{
				if(err)reject(err)
				resolve(rows[0].isDone)
				log.info(rows[0].isDone)
			})
		})		
	})
	return Promise.all(promises)
