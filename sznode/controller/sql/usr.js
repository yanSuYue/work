...
function queryUserInfoById(id){
	return new Promise((resolve,reject)=>{
		pool.query(`SELECT id FROM ROMS_SZ_SUBCOMPANY WHERE subCompanyName='深圳'`,(err,rows,field)=>{
			if(err)reject(err)
			resolve(rows[0])
			log.info(rows[0])
		})
...
function getstaff(id){
	let sql = `SELECT b.staffName , b.id FROM ROMS_SZ_DOC a JOIN ROMS_SZ_STAFF b ON a.staffAuditorId =b.id WHERE a.taskId='${id}'`
	return new Promise((resolve,reject)=>{
		pool.query(sql,[],(err,rows,field)=>{
